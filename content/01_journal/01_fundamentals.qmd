...
sp_500_prices_tbl <- readRDS("sp_500_prices_tbl.rds")
sp_500_prices_tbl

```{r}
sp_500_index_tbl <- readRDS("sp_500_index_tbl.rds")
sp_500_index_tbl
```
...
#combine kmeans and umap
```{r}
kmeans_obj <- kmeans_mapped_tbl %>%
  pull(k_means) %>%
  pluck(10)
  
combined_result_tbl <- kmeans_obj %>%
...
```{r}
ggplot(combined_result_tbl, aes(x = V1, y = V2, color = factor(.cluster))) +
geom_point(alpha = 0.5) +
